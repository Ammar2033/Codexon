...
import { ContainerInfo, allocateContainer, destroyContainer, getContainerInfo, recordRequest, getAllContainers } from './container_allocator';
...
export async function acquireContainer(modelId: string, config: ContainerConfig): Promise<ContainerInfo> {
  ...
  const available = pool.find(c => !c.isWarming && c.info.status === 'ready');
  
  if (available) {
    available.idleSince = new Date();
  ...
  if (diff > 0) {
    for (let i = 0; i < diff; i++) {
      try {
        ...
        const info = await allocateContainer({ ...config, port });
        pool.push({ info, idleSince: new Date(), isWarming: false });
      } catch (error) {
